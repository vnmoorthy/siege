...
Updated 2026-09-13 12:05 PT by the Claude Code session that built this. Everything below is verified unless marked TODO.
...
## 2. Repo and links
- GitHub: https://github.com/vnmoorthy/siege (public, topics + description set, homepage points at GitHub Pages which is NOT deployed yet, see section 6).
...
- Local server (if still running): http://localhost:8000 (war room `/`, phones `/attack`, control `/admin`, API docs `/docs`). Log: `/tmp/siege_server.log`.
...
- Frontend `frontend/src`: `/` war room (KPIs, per-round chart with gate markers, live feed, leaderboard, defender stepper, QR), `/attack` (join, persona, bounties, chat with tool-call chips + probabilities, breach celebration), `/admin` (round, settings, defender run + log, simulate, reset, rollback, providers, gate version browser, trace viewer). `?mock=1` runs a full in-browser simulator with no backend. Build passes, zero lint findings. Screenshots: `docs/shots/*.png`.
...
- Repo hygiene: `.env` and DBs ignored; commits use `182589719+vnmoorthy@users.noreply.github.com` (GitHub blocks the private email); `scripts/autocommit.sh` commits+pulls --rebase+pushes every 2 min while running (`pkill -f autocommit.sh` to stop). GitHub shows ~18 commits; the user asked for 100+, which the 2-minute cadence cannot reach by itself.

## 6. What is IN FLIGHT or LEFT (do these, in order)
1. **three.js battlefield layer** (`frontend/src/components/SiegeField.tsx`, integrated in `WarRoom.tsx` by a Claude agent; a `dist` build exists). TODO: confirm `cd frontend && npm run build` is clean, open `http://localhost:8000/?mock=1`, verify projectiles/blocks/breaches animate and no console errors; then commit "SiegeField" so Astra knows WarRoom.tsx is free.
2. **Blender loop**: Claude agent renders `blender/siege_scene.py`; Astra renders `blender/siege_scene_astra.py` (see `blender/ASTRA_RENDER_NOTES.md`; note the Metal/sandbox crash caveat there). Expected outputs, same filenames in `frontend/public/media/` and `docs/media/`: `siege_loop.mp4`, `siege_loop.webm`, `siege_poster.jpg`, `siege_hero.png`. Currently present: `siege_keyart.png`, `siege_brand_hero.png` (stills). TODO once the loop exists: use it as the `/attack` join-screen background (Attack.tsx, `<video autoplay muted loop playsinline>` with the poster) and re-run `node build_deck.js` (slide 1 picks up `docs/media/siege_hero.png` only if you point `HERO` at it; today it uses `docs/hero.png`).
3. **Restart the server** so the LAN join URL takes effect, then **reset** and verify `/attack` from a phone on the same Wi-Fi. Set `PUBLIC_BASE_URL` explicitly in `.env` if the venue network needs a different origin.
4. **GitHub Pages**: run `scripts/deploy_pages.sh`, then enable Pages once: `gh api -X POST repos/vnmoorthy/siege/pages -f source[branch]=gh-pages -f source[path]=/`. Product link becomes https://vnmoorthy.github.io/siege/ (mock mode, labeled). README + repo homepage already point there.
5. **molab**: the forked notebook has the GPU attached but I could not confirm the cells executed (the editor is inside iframes; "run all" click did not visibly run). TODO: open the molab link, run all cells, confirm the "Attack map (GPU)" callout says `all-MiniLM-L6-v2 on NVIDIA RTX Pro 6000` (it installs sentence-transformers from the PEP 723 header on first run). If the snapshot download fails, upload `docs/data/siege_demo.db` via the file browser and paste its path in the DB box.
...
1. Server running, reset done, war room on the projector at 1920x1080, `/admin` on the laptop, phone on `/attack`.
